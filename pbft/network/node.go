...
type Node struct {
	...
	StableCheckPointMutex sync.RWMutex
...
func NewNode(myInfo *NodeInfo, nodeTable []*NodeInfo, seedNodeTables [][]*NodeInfo,
	...
	for i := 0; i < len(node.NodeTable); i++ {
		go node.dispatchMsg()
	}
...
func (node *Node) Broadcast(msg interface{}, path string) {
	...
	node.MsgOutbound <- &MsgOut{Msg: jsonMsg, Path: path}
}
...
func (node *Node) GetCollate(state consensus.PBFT, collateMsg *consensus.CollateMsg) {
	...
	// Log last sequence id for checkpointing
	node.PreparedMutex.RLock()
	preparedstatus := node.Prepared[collateMsg.SequenceID]
	node.PreparedMutex.RUnlock()
...
func (node *Node) executeMsg() {
	...
	for {
		...
		complete[prepareMsg.SequenceID] = 1
		//fmt.Println("/[CommitMsg] /", node.MyInfo.NodeID, "/", prepareMsg.SequenceID, "/", time.Now().UnixNano())
		//log.Println("/[CommitMsg] /", node.MyInfo.NodeID, "/", prepareMsg.SequenceID, "/", time.Now().UnixNano())

		var lastSequenceID int64
		node.StableCheckPointMutex.RLock()
		lastSequenceID = node.StableCheckPoint
		node.StableCheckPointMutex.RUnlock()
		for {
			if complete[lastSequenceID+1] == 1 {
				lastSequenceID += 1

				//fmt.Println("/[Execute] /", node.MyInfo.NodeID, "/", lastSequenceID, "/", time.Now().UnixNano())
				//log.Println("/[Execute] /", node.MyInfo.NodeID, "/", lastSequenceID, "/", time.Now().UnixNano())

				node.StatesMutex.RLock()
				state := node.States[lastSequenceID]
				node.StatesMutex.RUnlock()

				node.CommittedMsgs[lastSequenceID] = state.GetPrepareMsg()

				ch := state.GetMsgExitSendChannel()
				ch <- 0
			} else {
				node.StableCheckPointMutex.Lock()
				node.StableCheckPoint = lastSequenceID
				node.StableCheckPointMutex.Unlock()
				break
			}
