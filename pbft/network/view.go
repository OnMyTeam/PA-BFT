...
func (node *Node) getPrimaryInfoByID(sequenceID int64) *NodeInfo {
	...
	seqID := int(sequenceID % 10)
	if seqID == 0 {
		seqID = 10
	}
	// divnum := len(node.NodeTable)/7
	// nodenum := (seqID*(divnum+1)-(divnum))%len(node.NodeTable)
	// if nodenum == 0{
	// 	nodenum = len(node.NodeTable)
	// }
	// primary := node.NodeTable[nodenum-1]
	divnum := 2*seqID - 1
	nodenum := 1
	if divnum == 19 {
		nodenum = 10
